...
mod tests {
    ...
    async fn stillness_uses_overridden_tolerance() {
        ...
        // Drift 2mm — inside the configured default (6mm) but past the 1mm
        // override — then let it settle (the round re-centers on the new
        // spot after a slip, so only the one drift should cost a life).
        ...
        let g = state.read().await.game.clone();
        assert_eq!(g.level, 4, "one life lost out of the default 5 from the 1mm-override drift");
