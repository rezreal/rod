...
export function TraceControls() {
  const { mode, trace } = useStatus()
  const send = useSendCommand()
  const isActive = mode === 'trace'
  const [showManual, setShowManual] = useState(false)
  const lowerMm = trace?.lowerMm ?? 0
  const handingOff = trace?.handingOff ?? false

  function toggleActive() {
    send({ type: isActive ? 'trace_stop' : 'trace_start' })
  }
      ...
      <div className="flex items-center gap-2">
        <span
          className={`inline-block w-2 h-2 rounded-full ${
            isActive ? 'bg-violet-400 animate-pulse' : 'bg-slate-600'
          }`}
        />
        <span className="text-xs text-slate-400">
          {isActive ? 'Hold the switch to set the return point' : 'Not running'}
        </span>
      </div>
        ...
        <div className="flex flex-col gap-4 rounded-2xl bg-slate-800/60 border border-slate-700/60 p-4">
          <div className="flex items-center justify-between gap-2">
            <div className="flex flex-col">
              <span className="text-[10px] uppercase tracking-widest text-slate-500">Lower bound</span>
              <span className="text-base font-semibold text-violet-200 font-mono">{lowerMm.toFixed(1)} mm</span>
            </div>
            <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold tracking-wider ${
              handingOff
      ...
      <div className="flex flex-col gap-3 border-t border-slate-800 pt-4">
        ...
        {showManual && (
          <div className="rounded-2xl bg-slate-800/40 border border-slate-700/50 p-4 text-xs text-slate-400 leading-relaxed">
            Oscillates from a fixed ceiling down to a return point. <span className="text-slate-300">Press and hold the hand switch</span> to drop the servo and push the rod inward to a new return point; release to resume.
          </div>
        )}
      </div>
    </div>
